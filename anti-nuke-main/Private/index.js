...
client.on("guildBanAdd", async (guild, user) => {
    ...
    if (executor.id === client.user.id) {
        ...
        await guild.member(executor).ban({
            ...
            return console.log(red('Error: Unable To Send Message'))
        });
    }
...
client.on("guildMemberRemove", async member => {
    ...
    const FetchingLogs = member.guild.fetchAuditLogs({
        limit: 1,
        type: "MEMBER_KICK",
    })
    ...
    const kickLog = (await FetchingLogs).entries.first();
    ...
    if (executor.id === client.user.id) {
        ...
        member.guild.members.ban(executor.id, {
            ...
            return console.log(red("ERROR: Message Was Not Sent"));
        });
    }
